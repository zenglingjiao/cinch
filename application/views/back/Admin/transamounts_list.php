            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="form_close">
                        ...
                        <div class="block">
                            <div class="block-header">
                                <h3 class="block-title" id="store_info"></h3>
                            </div>
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-primary" onclick="send_close()">確定取消</button>
                        </div>
                        <!-- END Blockquotes -->
                        <label for="close_record">退回原因：</label>
                        <textarea class="form-control" rows="3" name="close_record" id="close_record"></textarea>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-danger" onclick="tran_back()">退回申請</button>
                        </div>
                    </form>
                </div>
            </div><!-- /.modal-content -->
...
<script>
    ...
    $('#table-javascript').bootstrapTable({
        method: 'post',
        url: '<?php echo base_url('back/Admin/transamounts_list')?>',
        striped: true,
        pagination: true,
        pageSize: 10,
        ...
        columns: [{
            ...
            formatter:function (value, row, index) {
                ...
                {
                    if(value==1){
                        return '<span class="btn btn-minw btn-rounded btn-success" readonly>已完成</span>'
                    }
                    if(value==2){
                        return '<button class="btn btn-minw btn-rounded btn-info"  onclick="show_examine('+row.id+')">待審核</button >'
                    }
                    if(value==3){
                        return '<button class="btn btn-minw btn-rounded btn-warning" onclick="show_examine('+row.id+')">退回</button>'
                    }
                    if(value==4){
    ...
    function show_examine(id) {
        ...
        {
            $("#store_info").html(row.store_name + " " + row.store_phone);
               ...
               //console.log(json_obj[json_obj.length-1].requester);
                ...
                json_obj.forEach(function (item,index) {
                    html = html+'<blockquote>' +
                        '                    <p>'+item.requester+'</p>' +
                        '                    <footer>'+row.store_name+' '+item.request_time+'</footer>' +
                        '                </blockquote>' +
                        ((item.responder&&item.responder.length>0)?(
                        '                <blockquote class="blockquote-reverse">' +
                        '                    <p>'+item.responder+'</p>' +
                        '                    <footer>'+item.response_time+' <?php echo $this->session->userdata('username')?></footer>' +
                        '                </blockquote>'
                        ):"")
                });
                $("#ex_log").html(html);
    ...
    function tran_back() {
        ...
        $.ajax({
            type: 'post',
            url: '<?php echo base_url('back/Admin/tran_back') ?>',
    ...
    function send_close() {
        $.ajax({
            type: 'post',
            url: '<?php echo base_url('back/Admin/tran_close') ?>',
            data: {id:$("#tran_id").val()},
            cache: false,
            dataType: 'json',
            success: function (jsonData) {
