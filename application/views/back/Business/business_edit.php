            ...
            <div class="row items-push">
                ...
                <div class="col-sm-5 text-right hidden-xs">
                    <ol class="breadcrumb push-10-t">
                        ...
                        <li><a class="link-effect" href="<?php echo base_url('back/Business/business_list')?>">商家列表</a></li>
                            ...
                            <div class="block-content block-content-narrow">
                                <form id="addform" class="js-validation-bootstrap form-horizontal" action="" method="post" novalidate="novalidate" autocomplete="off">
                                    ...
                                    <div class="form-group" v-if="!model.id>0">
                                        <label class="col-md-2 control-label">商家類別</label>
                                        <div class="col-md-8">
                                            <label class="css-input css-radio css-radio-warning push-10-r">
                                                <input type="radio" value="0" v-model="model.type" name="type"><span></span> 單一商店
                                            </label>
                                            <label class="css-input css-radio css-radio-warning push-10-r">
                                                <input type="radio" value="1" v-model="model.type" name="type"><span></span> 總店
                                            </label>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-2">
                                            <select class="form-control" name="county" id="county" placeholder="縣市" @change="selected_county($event)">
                                                <option value="">選擇縣市</option>
                                                <option v-for="c in county" :value="c.id" :selected="model.county==c.id">{{c.name}}</option>
                                            </select>
                                        </div>
                                        <div class="col-md-2">
                                            <select class="form-control" name="district" id="district" placeholder="區" @change="selected_district($event)">
                                                <option value="">選擇區</option>
                                                <option v-for="d in district" :value="d.id" :selected="model.district==d.id">{{d.name}}</option>
                                            </select>
                                        </div>
                                        <div class="col-md-4">
                                    ...
                                    <div class="form-group">
                                        <label class="col-md-2 control-label">郵遞區號</label>
                                        <div class="col-md-4">
                                            <input class="form-control" type="text" @change="postal_change" name="postal"  id="postal" v-model="model.postal" placeholder="郵遞區號">
                                        </div>
                                    </div>
                                    <div class="form-group">
                                    ...
                                    <input type="hidden" name="longitude" v-model="model.longitude">
                                    <input type="hidden" name="latitude" v-model="model.latitude">
...
<script>
    ...
    var vue_obj = new Vue({
        ...
        mounted: function () {
            ...
            if (json_model) {
                ...
                if (json_model.address_list && json_model.address_list.length > 0) {
                    this.county = json_model.address_list.filter(function (o) {
                        return (o.level == 2);
                    });
                    if(json_model.district&&json_model.district.length>0)
                    {
                        this.district = json_model.address_list.filter(function (o) {
                            return (o.parent == json_model.county);
                        });
                    }
                }
            }
        ...
        methods: {
            ...
            selected_county: function (event) {
                var el = event.currentTarget;
                this.model.county = el.value;
                this.district = this.model.address_list.filter(function (o) {
                    return (o.parent == el.value);
                });
                $("#district").val("");
            },
            selected_district: function (event) {
                var el = event.currentTarget;
                this.model.district = el.value;
                this.model.address = $("#county").find('option:selected').text() + $("#district").find('option:selected').text();
            },
            postal_change:function () {
                this.model.postal = this.model.postal.replace(/[^\d]/g,'');
            }
        },
        // 使用一个监听。可以减少很多代码
        watch: {
    ...
    function SendAjax() {
        ...
        $.ajax({
            type: 'post',
            url: '<?php echo base_url('back/Business/business_edit') ?>',
            data: fd,
            cache: false,
