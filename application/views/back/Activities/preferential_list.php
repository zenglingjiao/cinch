        ...
        <div class="content bg-gray-lighter">
            <div class="row items-push">
                ...
                <div class="col-sm-5 text-right hidden-xs">
                    <a href="<?php echo base_url('back/Activities/preferential_edit')?>" class="btn btn-primary" title="新增">新增</a>
                </div>
            </div>
        </div>
        <!-- END Page 中間標題頭部 -->
            ...
            <div class="message-table table-responsive smart-widget">
                <div class="smart-widget-header">
                    <div class="form-inline no-margin">
                        <div style="">
                            <div class="form-group">
                                <button type="button" class="btn btn-danger" onclick ="Ajax_Post_Select_msg('刪除選定項？ 刪除後無法還原！', '<?php echo base_url('back/Activities/preferential_delete')?>', 'table-javascript', '至少選擇一條')">選擇刪除</button>
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    $('#table-javascript').bootstrapTable({
        method: 'post',
        url: '<?php echo base_url('back/Activities/preferential_list')?>',
        striped: true,
        pagination: true,
        pageSize: 10,
        ...
        onClickRow: function (row, $element, field) {
            if (field == "STATE" || field == "id" || field == "name"|| field == "status") { } else {
                window.location.href = "<?php echo base_url('back/Activities/preferential_edit/')?>" + row.id;
            }
        },
        onLoadSuccess: function () {
            $('.xcheck').change(function () {
                ...
                $.ajax({
                    ...
                    data: 'tb=preferentials&id=' + this.value + '&set=' + poststate+ '&field=status',
                    //async : false, //同步方式
