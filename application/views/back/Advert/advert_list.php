        ...
        <div class="content bg-gray-lighter">
            <div class="row items-push">
                ...
                <div class="col-sm-5 text-right hidden-xs">
                    <a href="<?php echo base_url('back/Advert/advert_edit')?>" class="btn btn-primary" title="新增">新增</a>
                </div>
            </div>
        </div>
        <!-- END Page 中間標題頭部 -->
                            ...
                            <form class="form-inline no-margin herd-from-alert" id="form_search">
                                <div class="cleom">
                                    ...
                                    <div class="form-group">
                                        <div class="form-material floating">
                                            <select name="type" id="type" class="form-control">
                                                <option value=""></option>
                                                <option value="0">內部商家</option>
                                                <option value="1">內部活動</option>
                                                <option value="2">外部鏈接</option>
                                            </select>
                                            <label for="type">廣告類型</label>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <div class="form-material floating">
                                    ...
                                    <div class="form-group">
                                        <div class="form-material">
                                            <div class="input-group">
                                                <input placeholder="上架時間-開始" name="startshelf_at" id="startshelf_at" class=" form-control dateStart" type="text" readonly>
                                                <div class="input-group-addon clearBtns">x</div>
                                            </div>
                                            <label for="startshelf_at">上架時間-開始</label>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <div class="form-material">
                                            <div class="input-group">
                                                <input placeholder="下架時間-結束" name="endshelf_at" id="endshelf_at" class=" form-control dateStart" type="text" readonly>
                                                <div class="input-group-addon clearBtns">x</div>
                                            </div>
                                            <label for="endshelf_at">下架時間-結束</label>
                                        </div>
                                    </div>
            ...
            <div class="message-table table-responsive smart-widget">
                <div class="smart-widget-header">
                    <div class="form-inline no-margin">
                        <div style="">
                            <div class="form-group">
                                <button type="button" class="btn btn-danger" onclick ="Ajax_Post_Select_msg('刪除選定項？ 刪除後無法還原！', '<?php echo base_url('back/Advert/advert_delete')?>', 'table-javascript', '至少選擇一條')">選擇刪除</button>
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    $(function () {
        ...
        calenders("#startshelf_at", true, false);
        calenders("#endshelf_at", true, false);

    });

    $('#table-javascript').bootstrapTable({
        method: 'post',
        url: '<?php echo base_url('back/Advert/advert_list')?>',
        striped: true,
        pagination: true,
        pageSize: 10,
        ...
        onClickRow: function (row, $element, field) {
            if (field == "STATE" || field == "id" || field == "name"|| field == "status") { } else {
                window.location.href = "<?php echo base_url('back/Advert/advert_edit/')?>" + row.id;
            }
        },
        onLoadSuccess: function () {
            $('.xcheck').change(function () {
                ...
                $.ajax({
                    ...
                    data: 'tb=advertisements&id=' + this.value + '&set=' + poststate+ '&field=status',
                    //async : false, //同步方式
        ...
        columns: [{
            ...
            sortable: true,
            formatter:function (value, row, index) {
                return getDate(row.startshelf_at)+" ~ "+getDate(row.endshelf_at);
            }
        }, {
            field: 'status',
            title: '關閉/開啟',
