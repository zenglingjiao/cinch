 ...
 * Class Goods
 */
class Prediction_win_image extends Admin_Controller
{
     ...
     * 狀態標籤管理
     */
    public function prediction_win_image_list()
    {
        if (IS_POST) {
//            return;
            ...
            $field = array(
                'prediction_win_image.id',
                'prediction_win_image.title',
                'prediction_win_image.imgs',
                'prediction_win_image.activities_time_start',
                'prediction_win_image.activities_time_end',
                'prediction_win_image.created_at',
                'prediction_win_image.state',
                'prediction_win_image.updated_at'
            );
            ...
            if ($c_time != "") {
                ...
                $this->db->where('prediction_win_image.created_at >=', $c_time[0]);
                //日期必須小於加一天的。 有時間<=就好，不需要加一天。
                $this->db->where('prediction_win_image.created_at <', date("Y-m-d", strtotime("+1 day", strtotime($c_time[1]))));
            }
            ...
            $data = $this->Data_helper_model->get_tabel_list($this->db, $_POST, "prediction_win_image");
            ...
            $data['active_prediction_win_image'] = "active";
            $data['h_title'] = "預測贏家圖片管理";
            $data['edit'] = base_url('back/Prediction_win_image/prediction_win_image_edit/');
            $data['api_list'] = base_url('back/Prediction_win_image/prediction_win_image_list');
            $data['api_delete'] = base_url('back/Prediction_win_image/prediction_win_image_delete');
            $data['state'] = base_url('back/Prediction_win_image/prediction_win_image_state');
            $this->load->view("back/Prediction_win_image/prediction_win_image_list", $data);
        }
    }
     ...
     * 狀態標籤編輯
     */
    public function prediction_win_image_edit($id = "")
    {
        if (IS_GET) {
            $data['open_challenge'] = "open";
            $data['active_prediction_win_image'] = "active";
            $data['list'] = base_url('back/Prediction_win_image/prediction_win_image_list');
            $data['edit'] = base_url('back/Prediction_win_image/prediction_win_image_edit/');
            $data['list_title'] = "預測贏家圖片列表";
            if (!empty($id)) {
                $id = (int)$id;
                $data['title'] = "預測贏家圖片編輯";
...
//                    var_dump($data);exit();
                    //$data['model'] = $model;
                    $this->load->view("back/Prediction_win_image/prediction_win_image_edit", $data);
                } else {
                    return_get_msg("信息錯誤", base_url('back/Prediction_win_image/prediction_win_image_list'));
                }
            } else {
                $data['title'] = "新增預測贏家圖片";
                $this->load->view("back/Prediction_win_image/prediction_win_image_edit", $data);
...
//            return;
            ...
            if (isset($_FILES) && count($_FILES) > 0) {
                ...
                foreach ($_FILES as $k => $file) {
                    if (isset($file['name'])) {
                        $path = "updata/Prediction_win_image/" . date("Y-m", time());
            ...
            if (!empty($id)) {
                ...
                if ($this->Data_helper_model->update_table_in_fileds(
                    "prediction_win_image",
                    ["id"],
                    [$id],
                    $sql_data
                )) {
                    return_post_json("ok", "修改成功", base_url('back/Prediction_win_image/prediction_win_image_list'), null);
                } else {
                    get_csrf_nonce_keep();
                ...
                if ($this->db->insert("prediction_win_image", $sql_data)) {
                    return_post_json("ok", "新增成功", base_url('back/Prediction_win_image/prediction_win_image_list'), null);
                } else {
     ...
     * 標籤管理狀態
     */
    public function prediction_win_image_state()
    {
     ...
     * 刪除狀態標籤
     */
    public function prediction_win_image_delete()
    {
        ...
        if (!empty($selectid)) {
            ...
            foreach ($id_list as $val) {
                if ($this->Data_helper_model->del_model_in_id('prediction_win_image', $val)) {
                    $is_ok++;
                } else {
                    $is_err++;
                }
