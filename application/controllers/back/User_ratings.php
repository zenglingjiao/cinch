 ...
 * Class Goods
 */
class User_ratings extends Admin_Controller
{
    ...
    public function ratings_list()
    {
        if (IS_POST) {
            ...
            if ($c_time != "") {
                ...
                $this->db->where('user_ratings.created_at <', date("Y-m-d", strtotime("+1 day", strtotime($c_time[1]))));
            }
            ...
            $data = $this->Data_helper_model->get_tabel_list($this->db, $_POST, "user_ratings");
            echo json_encode($data);
            //echo $this->db->last_query();
            exit;
        } else {
            $data['title'] = "用戶評級列表";
            $data['open_ratings'] = "open";
            $data['active_ratings'] = "active";
            $data['h_title'] = "用戶評級管理";
            $data['edit'] = base_url('back/User_ratings/ratings_edit/');
            $data['api_list'] = base_url('back/User_ratings/ratings_list');
            $data['api_delete'] = base_url('back/User_ratings/ratings_delete');
            $data['state'] = base_url('back/User_ratings/ratings_state');
            $this->load->view("back/User_ratings/ratings_list", $data);
        }
    }
     ...
     * 狀態標籤編輯
     */
    public function ratings_edit($id = "")
    {
        if (IS_GET) {
            $data['open_ratings'] = "open";
            $data['active_ratings'] = "active";
            $data['list'] = base_url('back/User_ratings/ratings_list');
            $data['edit'] = base_url('back/User_ratings/ratings_edit/');
            $data['list_title'] = "用戶評級列表";
            if (!empty($id)) {
                $id = (int)$id;
                $data['title'] = "用戶評級編輯";
                $model = $this->Data_helper_model->get_model_in_id("user_ratings", $id);
                if (isset($model)) {
                    $data['model'] = [
                        "id" => $model->id,
                        "title" => $model->title,
                    ...
                    //$data['model'] = $model;
                    $this->load->view("back/User_ratings/ratings_edit", $data);
                } else {
                    return_get_msg("信息錯誤", base_url('back/User_ratings/ratings_list'));
                }
            } else {
                $data['title'] = "新增用戶評級";
                $this->load->view("back/User_ratings/ratings_edit", $data);
...
//            return;
            ...
            if (isset($_FILES) && count($_FILES) > 0) {
                ...
                foreach ($_FILES as $k => $file) {
                    if (isset($file['name'])) {
                        $path = "updata/user_ratings/" . date("Y-m", time());
            ...
            if (!empty($id)) {
                ...
                if ($this->Data_helper_model->update_table_in_fileds(
                    ...
                    return_post_json("ok", "修改成功", base_url('back/User_ratings/ratings_list'), null);
                } else {
                    get_csrf_nonce_keep();
                ...
                if ($this->db->insert("user_ratings", $sql_data)) {
                    return_post_json("ok", "新增成功", base_url('back/User_ratings/ratings_list'), null);
                } else {
    ...
    public function ratings_state()
    {
        ...
        if ($id != "" && $field != "") {
            if ($this->Data_helper_model->tabel_status($id, "user_ratings", $field, $value)) {
    ...
    public function ratings_delete()
    {
        ...
        if (!empty($selectid)) {
            ...
            foreach ($id_list as $val) {
                if ($this->Data_helper_model->del_model_in_id('user_ratings', $val)) {
                    $is_ok++;
                } else {
                    $is_err++;
                }
