 ...
 * Class Goods
 */
class Exchange_activities extends Admin_Controller
{
...
//            return;
            ...
            $field = array(
                'exchange_activities.id',
                'exchange_activities.title',
                'exchange_activities.sort',
                'exchange_activities.imgs',
                'exchange_activities.activities_time_start',
                'exchange_activities.activities_time_end',
                'exchange_activities.created_at',
                'exchange_activities.state',
                'exchange_activities.updated_at'
            );
            ...
            if ($c_time != "") {
                ...
                $this->db->where('Exchange_activities.created_at <', date("Y-m-d", strtotime("+1 day", strtotime($c_time[1]))));
            }
            ...
            $data = $this->Data_helper_model->get_tabel_list($this->db, $_POST, "exchange_activities");
            echo json_encode($data);
            //echo $this->db->last_query();
            exit;
        } else {
            $data['title'] = "兌換活動列表";
            $data['open_activities'] = "open";
            $data['active_activities'] = "active";
            $data['h_title'] = "兌換活動管理";
            $data['edit'] = base_url('back/Exchange_activities/activities_edit/');
            $data['api_list'] = base_url('back/Exchange_activities/activities_list');
            $data['api_delete'] = base_url('back/Exchange_activities/activities_delete');
            $data['state'] = base_url('back/Exchange_activities/activities_state');
            $this->load->view("back/Exchange_activities/activities_list", $data);
        }
    }
    ...
    public function activities_edit($id = "")
    {
        if (IS_GET) {
            $data['open_activities'] = "open";
            $data['active_activities'] = "active";
            $data['list'] = base_url('back/Exchange_activities/activities_list');
            $data['edit'] = base_url('back/Exchange_activities/activities_edit/');
            $data['list_title'] = "兌換活動列表";
            if (!empty($id)) {
                $id = (int)$id;
                $data['title'] = "兌換活動編輯";
                $model = $this->Data_helper_model->get_model_in_id("exchange_activities", $id);
                if (isset($model)) {
                    $data['model'] = [
                        "id" => $model->id,
                        "title" => $model->title,
...
//                    var_dump($data);exit();
                    //$data['model'] = $model;
                    $this->load->view("back/Exchange_activities/activities_edit", $data);
                } else {
                    return_get_msg("信息錯誤", base_url('back/Exchange_activities/activities_list'));
                }
            } else {
                $data['title'] = "新增兌換活動";
                $this->load->view("back/Exchange_activities/activities_edit", $data);
...
//            return;
            ...
            if (isset($_FILES) && count($_FILES) > 0) {
                ...
                foreach ($_FILES as $k => $file) {
                    if (isset($file['name'])) {
                        $path = "updata/Exchange_activities/" . date("Y-m", time());
            ...
            if (!empty($id)) {
                ...
                if ($this->Data_helper_model->update_table_in_fileds(
                    ...
                    return_post_json("ok", "修改成功", base_url('back/Exchange_activities/activities_list'), null);
                } else {
                    get_csrf_nonce_keep();
                ...
                if ($this->db->insert("exchange_activities", $sql_data)) {
                    return_post_json("ok", "新增成功", base_url('back/Exchange_activities/activities_list'), null);
                } else {
     ...
     * 標籤管理狀態
     */
    public function activities_state()
    {
        ...
        if ($id != "" && $field != "") {
            if ($this->Data_helper_model->tabel_status($id, "exchange_activities", $field, $value)) {
    ...
    public function activities_delete()
    {
        ...
        if (!empty($selectid)) {
            ...
            foreach ($id_list as $val) {
                if ($this->Data_helper_model->del_model_in_id('exchange_activities', $val)) {
                    $is_ok++;
                } else {
                    $is_err++;
                }
