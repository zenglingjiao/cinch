 ...
 * Class Goods
 */
class Propaganda_film extends Admin_Controller
{
     ...
     * 狀態標籤管理
     */
    public function propaganda_film_list()
    {
        if (IS_POST) {
//            return;
            ...
            $field = array(
                'propaganda_film.id',
                'propaganda_film.title',
                'propaganda_film.sort',
                'propaganda_film.url',
                'propaganda_film.created_at',
                'propaganda_film.state',
                'propaganda_film.updated_at'
            );
            ...
            if ($c_time != "") {
                ...
                $this->db->where('propaganda_film.created_at >=', $c_time[0]);
                //日期必須小於加一天的。 有時間<=就好，不需要加一天。
                $this->db->where('propaganda_film.created_at <', date("Y-m-d", strtotime("+1 day", strtotime($c_time[1]))));
            }
            ...
            $data = $this->Data_helper_model->get_tabel_list($this->db, $_POST, "propaganda_film");
            ...
            $data['title'] = "宣傳影片列表";
            $data['open_home'] = "open";
            $data['active_propaganda_film'] = "active";
            $data['h_title'] = "宣傳影片管理";
            $data['edit'] = base_url('back/Propaganda_film/propaganda_film_edit/');
            $data['api_list'] = base_url('back/Propaganda_film/propaganda_film_list');
            $data['api_delete'] = base_url('back/Propaganda_film/propaganda_film_delete');
            $data['state'] = base_url('back/Propaganda_film/propaganda_film_state');
            $this->load->view("back/Propaganda_film/propaganda_film_list", $data);
        }
    }
     ...
     * 狀態標籤編輯
     */
    public function propaganda_film_edit($id = "")
    {
        if (IS_GET) {
            $data['open_home'] = "open";
            $data['active_propaganda_film'] = "active";
            $data['list'] = base_url('back/Propaganda_film/propaganda_film_list');
            $data['edit'] = base_url('back/Propaganda_film/propaganda_film_edit/');
            $data['list_title'] = "宣傳影片列表";
            if (!empty($id)) {
                $id = (int)$id;
                $data['title'] = "宣傳影片編輯";
                $model = $this->Data_helper_model->get_model_in_id("propaganda_film", $id);
                if (isset($model)) {
                    $data['model'] = [
                        "id" => $model->id,
                        "title" => $model->title,
...
//                    var_dump($data);exit();
                    //$data['model'] = $model;
                    $this->load->view("back/Propaganda_film/propaganda_film_edit", $data);
                } else {
                    return_get_msg("信息錯誤", base_url('back/Propaganda_film/propaganda_film_list'));
                }
            } else {
                $data['title'] = "新增宣傳影片";
                $this->load->view("back/Propaganda_film/propaganda_film_edit", $data);
...
//            return;
            ...
            if (!empty($id)) {
                ...
                if ($this->Data_helper_model->update_table_in_fileds(
                    ...
                    return_post_json("ok", "修改成功", base_url('back/Propaganda_film/propaganda_film_list'), null);
                } else {
                    get_csrf_nonce_keep();
                ...
                if ($this->db->insert("propaganda_film", $sql_data)) {
                    return_post_json("ok", "新增成功", base_url('back/Propaganda_film/propaganda_film_list'), null);
                } else {
     ...
     * 標籤管理狀態
     */
    public function propaganda_film_state()
    {
     ...
     * 刪除狀態標籤
     */
    public function propaganda_film_delete()
    {
        ...
        if (!empty($selectid)) {
            ...
            foreach ($id_list as $val) {
                if ($this->Data_helper_model->del_model_in_id('propaganda_film', $val)) {
                    $is_ok++;
                } else {
                    $is_err++;
                }
