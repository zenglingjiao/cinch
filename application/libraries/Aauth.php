...
class Aauth
{
    ...
    public function __construct()
    {
        ...
        $this->aauth_db = $this->CI->db;//$this->CI->load->database($this->config_vars['db_profile'], TRUE);
    ...
    public function login($identifier, $pass, $remember = FALSE, $totp_code = NULL)
    {
        ...
        if ($this->config_vars['login_with_name'] == TRUE && !strstr($identifier, '@')) {
        ...
        if ($query->num_rows() > 0) {
            ...
            //$this->CI->session->set_userdata("verification_url","back/Admin/verification");
            return FALSE;
        }else{
            //$this->CI->session->unset_userdata('verification_url');
        }

        // if user is not ok
        $query = null;
        $query = $this->aauth_db->where($db_identifier, $identifier);
        $query = $this->aauth_db->where('is_ok', 0);
        //$query = $this->aauth_db->where('verification_code !=', '');
        $query = $this->aauth_db->get($this->config_vars['users']);

        if ($query->num_rows() > 0) {
            $this->error($this->CI->lang->line('aauth_error_account_not_ok'));
            //$this->CI->session->set_userdata("verification_url","back/Admin/verification");
            return FALSE;
        }else{
            //$this->CI->session->unset_userdata('verification_url');
        ...
        if ($query->num_rows() != 0 && $this->verify_password($password, $row->pass)) {
            ...
            //$groupObj = $this->get_group_user($row->id);
            // If email and pass matches
            // create session
            // login_fast 記得同步更新這個方法裡的session值
            $data = array(
                'id' => $row->id,
    ...
    public function store_login($identifier, $pass, $remember = FALSE, $totp_code = NULL)
    {
     ...
     * 修改管理員密碼1
     * @param string
     * @return bool Password change fails/succeeds
     */
    public function change_password($pass)
    {
        $userid = $this->CI->session->userdata('id');
        if (!isset($pass) || empty($pass) || !isset($userid) || empty($userid)) {
            return FALSE;
        }
        ...
        if (!$this->config_vars['use_password_hash']) {
            $pass = $this->hash_password($pass, $userid);
        } else {
     ...
     * @return bool Password change fails/succeeds
     */
    public function change_password_eidt($id, $pass)
    {
     ...
     * 判斷密碼是否正確
     */
    public function check_logged_user_password($pass)
    {
        $userid = $this->CI->session->userdata('id');
        // Update to correct salted password
        if (!$this->config_vars['use_password_hash']) {
        ...
        if(!empty($pass))
        {
            ...
            $query = $this->aauth_db->get($this->config_vars['users']);
            $row = $query->row();
            if($row&&$row->id==$userid&&$this->verify_password($pass, $row->pass))
            {
                return TRUE;
            }
        }
    ...
    public function create_user_role($post, $email, $pass, $name ,$store = false,$store_data = [])
    {
        ...
        if ($this->user_exist_by_username($name) && $name != FALSE) {
            $this->error($this->CI->lang->line('aauth_error_username_exists'));
            $valid = FALSE;
        }

        if ($this->user_exist_by_email($email) && $email != "") {
            $this->error($this->CI->lang->line('aauth_error_email_exists'));
            $valid = FALSE;
        }
        ...
        if ($store&&isset($store_data)&&count($store_data)>0) {
        ...
        if ($this->aauth_db->insert($this->config_vars['users'], $data)) {
            ...
            if ($store) {
                $this->add_member_null_create($user_id, "Store", "店鋪");
            }
            // if verification activated
            if ($this->config_vars['verification'] && !$this->is_admin()) {
                $data = null;
    ...
    public function update_user_rolename($user_id,$full_name, $email = FALSE, $pass = FALSE, $username = FALSE)
    {
        $data = array();
        $valid = TRUE;
        $user = $this->get_user($user_id);
    ...
    public function get_user_in_group($group)
    {
        $field = array(
            'aauth_users.id',
            'aauth_users.airportName',
            'aauth_users.saleName',
            'aauth_groups.name group_name',
            'aauth_groups.definition'
        );

        $this->aauth_db->select($field);
        $this->aauth_db->where('username !=', 'admin');
        $this->aauth_db->where('aauth_groups.name', $group);
        $this->aauth_db->join("aauth_user_to_group", 'aauth_user_to_group.user_id = aauth_users.id and aauth_user_to_group.group_id !=3', 'left');
        $this->aauth_db->join("aauth_groups", 'aauth_user_to_group.group_id = aauth_groups.id and aauth_groups.id !=3', 'left');
        $query = $this->aauth_db->get($this->config_vars['users']);
        return $query->result();
    }
    ...
    public function add_member_null_create($user_id, $group_name, $definition)
    {
        $group_id = $this->get_group_id($group_name);

        if (!$group_id) {
            $this->create_group($group_name, $definition);
        }
        $group_id = $this->get_group_id($group_name);
        if (!$group_id) {
            $this->error($this->CI->lang->line('aauth_error_no_group'));
    ...
    public function list_groups_user()
    {
        $field = array(
            'aauth_user_to_group.user_id',
            'aauth_user_to_group.group_id',
            'aauth_groups.name',
            'aauth_groups.definition'
        );

        $this->aauth_db->select($field);
        $this->aauth_db->from('aauth_user_to_group');

        //$this->db->join('tb_newsLike', 'tb_News.id = tb_newsLike.newsID', 'left');
        $this->aauth_db->join("aauth_groups", 'aauth_user_to_group.group_id = aauth_groups.id', 'left');
        $query = $this->aauth_db->get();
        return $query->result();
    }
    ...
    public function get_group_user($userid)
    {
        $this->aauth_db->where('user_id', $userid);
        $this->aauth_db->where('group_id !=', 3);
        $query = $this->aauth_db->get($this->config_vars['user_to_group']);
        $user_group = $query->row();
        if (isset($user_group)) {
            $query = $this->aauth_db->where('id', $user_group->group_id);
            $query = $this->aauth_db->get($this->config_vars['groups']);
            return $query->row();
        }
    ...
    /**
     * Is user allowed
     * 检测账号（默认检测当前登入账号）是否有权限  admin始终有权限
